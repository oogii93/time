    ...
    <div class="card">
        ...
        <div class="card-body">
            <div class="table-responsive">
                <table class=" table table-bordered table-striped table-hover datatable datatable-arrivalDepartureRecords">
                    <thead>
                        <tr>
                            <th width="10">
                            ...
                            <tr data-entry-id="{{ $departureRecord->id }}">
                                ...
                                <td>
                                    @can('departure_record_show')
                                        <a class="btn btn-xs btn-primary" href="{{ route('admin.departure-records.show', $departureRecord->id) }}">
                                            {{ trans('global.view') }}
                                        </a>
                                    @endcan

                                    @can('departure_record_edit')
                                        <a class="btn btn-xs btn-info" href="{{ route('admin.departure-records.edit', $departureRecord->id) }}">
                                            {{ trans('global.edit') }}
                                        </a>
                                    @endcan

                                    @can('departure_record_delete')
                                        <form action="{{ route('admin.departure-records.destroy', $departureRecord->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
...
@endcan
  ...
  let table = $('.datatable-arrivalDepartureRecords:not(.ajaxTable)').DataTable({ buttons: dtButtons })

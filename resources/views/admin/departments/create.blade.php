    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.departments.store") }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('staffs') ? 'is-invalid' : '' }}" name="staffs[]" id="staffs" multiple>
                    @foreach($staffs as $id => $staff)
                        <option value="{{ $id }}" {{ in_array($id, old('staffs', [])) ? 'selected' : '' }}>{{ $staff }}</option>
                    @endforeach
                </select>
